...
def _derive_volumes(persistent_paths: list[TracePath]) -> list[PolicyVolume]:
    ...
    Per spec §5.6: ``/var/lib/example-app`` and ``/var/log/example-app`` both want
    ``example-app-data``. The first (sorted by path) keeps the bare ``example-app-data``;
    the others get the parent leaf prepended (``log-example-app-data``).
    """
