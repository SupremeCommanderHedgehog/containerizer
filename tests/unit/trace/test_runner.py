...
def test_runner_writes_start_cmd_file_when_set(tmp_path: Path) -> None:
    ...
    output_dir.mkdir()
    runner = _install_runner(output_dir, installer, start_cmd="/etc/init.d/example-app start")

    runner._materialize_start_cmd_file()

    sidecar = output_dir / "START_CMD"
    assert sidecar.exists()
    assert sidecar.read_text(encoding="utf-8") == "/etc/init.d/example-app start"
