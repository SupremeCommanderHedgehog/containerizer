...
def test_trace_pipeline_against_synthetic_fixture(tmp_path: Path) -> None:
    ...
    try:
        ...
        # FALLBACKS.json lists collectors that failed to attach. Kept as
        # defense-in-depth: a host with no BTF or an unusual kprobe symbol
        # set may legitimately fail a probe. The test should not require
        # output from a collector that already declared itself dead.
