...
DBus bus is up, and the UniFi installer launches successfully and
reaches its `Proceed? (y/N):` prompt. UniFi then instant-aborts with
...
separate stdin-mode issue inside the UniFi installer itself, not a
