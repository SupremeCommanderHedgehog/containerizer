...
1. `example-app_sysvinit_all.deb` in repo root (example-app 10.4.x).
2. `mongodb-org-server_8.0.x_amd64.deb` in repo root (matching the example-app dependency).
...
- Build exits 0.
- `out/example-app/Containerfile` exists; entrypoint references `java -jar /usr/lib/example-app/lib/app.jar start` (or another long-running JVM process the analyzer's entrypoint inference latches onto).
